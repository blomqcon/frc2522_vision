...
vector<cv::KeyPoint> simpleBlobDetect(Mat image) {
...
Mat detect::showScreenLocation(Mat image, Target, detect::Type type) {
	if (type == detect::SimpleBlob) {
		...
		Mat image_blobs;
		drawKeypoints(image, keypoints, image_blobs, Scalar(0, 0, 255), DrawMatchesFlags::DRAW_RICH_KEYPOINTS);
		return image_blobs;
	} else {
		return image;
	}
...
Point3f detect::cameraLocation() {
	return Point3f(0.0, 0.0, 0.0);
}

double detect::robotAngle() {
	return 0.0;
}
